...
import discharge_model as discharge_model
import full_model as full_model
from rdp import rdp, rdp_fixed_points_exact
        ...
        for i in range(num_cells): # 該batch下的電池cell數量
            ...
            for j in range(1, cycle_length+1): # 選擇前n個cyle
                ...
                if mode=='interp':
                    ...
                    print(charge_info.shape)
                    print(discharge_info.shape)
                    ...
                    # 因各個曲線output 出的點數不同，需要再導入線性差值
                    charge_info = RDP([Qc[ch_s:ch_e], voltage[ch_s:ch_e], current[ch_s:ch_e], temper[ch_s:ch_e]])
                    discharge_info = RDP([Qd[dis_s:dis_e], voltage[dis_s:dis_e], current[dis_s:dis_e], temper[dis_s:dis_e]])
                    print(charge_info.shape)
                    print(discharge_info.shape)
                    cycle_info.append(np.expand_dims(np.hstack([charge_info, discharge_info]), axis=0))
                    temp = cycle_info[-1]
...
def data_visualization(f_id, cycles=2000):

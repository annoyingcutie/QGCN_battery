...
class BatteryRegressionDataset:
    def __init__(self, battery_data, eol_labels, n_wires):
        self.data = battery_data  # shape: (N, 4, T)
        self.labels = eol_labels  # shape: (N,)
        self.n_wires = n_wires
        self.states = []
        self.Xlabel = []

        for i in range(len(battery_data)):
            theta, phi = curve_to_theta_phi(battery_data[i])
            state = theta_phi_to_state(theta, phi, self.n_wires)
            self.states.append(state)
            self.Xlabel.append(eol_labels[i])  # 直接使用真實 EOL 當作 y

        self.states = np.array(self.states)
        self.Xlabel = np.array(self.Xlabel)
...
class QGCN(tq.QuantumModule):
    def __init__(self, g, n_feature, n_vqc_wires=4, add_fc=True):
        super().__init__()
        ...
        if add_fc:
            self.fc_layer = torch.nn.Linear(n_vqc_wires, 1)
...
def train(model, train_loader, epochs=20, device='cpu'):
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            loss = torch.nn.functional.mse_loss(output[batch.train_mask], batch.y[batch.train_mask])
...
def test(model, test_loader, device='cpu'):
    ...
    for batch in test_loader:
        ...
        loss = torch.nn.functional.mse_loss(output[batch.test_mask], batch.y[batch.test_mask])
        total_loss += loss.item()

        preds = output[batch.test_mask]
        #.argmax(dim=1)  # Get predictions
